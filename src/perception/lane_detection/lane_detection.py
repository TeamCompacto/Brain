...
def region_of_interest(frame):
    ...
    polygon = np.array([[
        ...
        (int(width*0.20),  int(height*0.65)),   # Top-left point
        (int(width*0.80), int(height*0.65)),    # Top-right point
...
def detect_lines(frame):    
    ...
    line_segments = cv2.HoughLinesP(frame, 1, np.pi/180 , 20, 
                                    np.array([]), minLineLength=70, maxLineGap=150)
...
def optimize_lines(frame, lines):
    ...
    if lines is not None:   # If there no lines we take line in memory
        ...
        for line in lines:  # Access each line in lines scope
            ...
            if slope < -0.15:   # Here we check the slope of the lines 
                #left_fit.append((slope, intercept))
                #cv2.line(frame, (x1, y1), (x2, y2), (255, 0, 0), 2)

                midpoint = abs(x2-x1)/2 + x2
                ...
                if midpoint > 120:
                    ...
                    right_y1 += y1
                    right_y2 += y2
                    #print(f"right : {midpoint}")

        if lefts > 0.01:
            left_x1 = int(left_x1 / lefts)
            left_x2 = int(left_x2 / lefts)
            left_y1 = int(left_y1 / lefts)
            left_y2 = int(left_y2 / lefts)

            cv2.line(frame, (left_x1, left_y1), (left_x2, left_y2), (0, 255, 255), 2)

            lane_lines.append([[left_x1, left_y1, left_x2, left_y2]])
            #print("NA@")
        else:
            lane_lines.append(None)

        if rights > 0.01:
            right_x1 = int(right_x1 / rights)
            right_x2 = int(right_x2 / rights)
            right_y1 = int(right_y1 / rights)
            right_y2 = int(right_y2 / rights)

            cv2.line(frame, (right_x1, right_y1), (right_x2, right_y2), (255, 255, 0), 2)

            lane_lines.append([[right_x1, right_y1, right_x2, right_y2]])
            #print("NA!")
        else :
            lane_lines.append(None)
...
# img = cv2.imread('camera_cal/csuf5.jpg')
# dev, prd = process_frame(img)
# print(dev)
# cv2.imwrite('prd.jpg', prd)